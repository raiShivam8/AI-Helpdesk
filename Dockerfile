...
CMD ["sh", "-c", "mkdir -p storage/framework/cache/data storage/framework/sessions storage/framework/views storage/logs bootstrap/cache && chmod -R 775 storage bootstrap/cache 2>/dev/null || true && php artisan storage:link --force 2>/dev/null || true && php artisan config:clear || true && php artisan route:clear || true && php artisan view:clear || true && php artisan package:discover --ansi || true && (php -d max_execution_time=0 artisan queue:work --sleep=3 --tries=3 --timeout=120 --max-time=3600 &) && (php artisan schedule:work &) && (sleep 2 && php artisan migrate --force &) && (php artisan serve --host=0.0.0.0 --port=80 &) && (php artisan serve --host=0.0.0.0 --port=8080 &) && php artisan serve --host=0.0.0.0 --port=${PORT:-8080}"]
