...
<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div class="space-y-4">
                ...
                @foreach($ticket->replies as $reply)
                    ...
                    {{-- ╔════════════════════════════════════╗
                         ...
                         ╚════════════════════════════════════╝ --}}
                    @if($reply->sender_type === \App\Enums\SenderType::Agent || $reply->sender_type === \App\Enums\SenderType::System)
        ...
        <div class="space-y-4">
            ...
            {{-- ── Assigned Agent ── --}}
            <div class="card p-5">
                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Assigned Agent</p>

                @if(auth()->user()->isAdmin())
                    <form method="POST" action="{{ route('tickets.assign', $ticket) }}" x-data x-ref="assignForm">
                        @csrf
                        @method('PATCH')
                        <select name="assigned_agent_id" id="assigned_agent_id"
                                @change="$refs.assignForm.submit()"
                                class="form-select">
